...
from tkinter import (
    Tk, Frame, Label, Button, Entry, StringVar, Text, END, filedialog, messagebox, ttk,
)
...
class MainWindow:
    ...
    def _build_data_tab(self):
        f = self.tab_data
        Label(f, text="Step 1 — Upload Market Data (CSV)", font=("", 13, "bold")).pack(pady=10, anchor="w", padx=10)
        Button(f, text="Browse for CSV...", command=self._browse_csv).pack(anchor="w", padx=10)
        self.data_status = Label(f, text="No file selected.", fg="#555")
        self.data_status.pack(anchor="w", padx=10, pady=6)
        Label(
            f,
            text="A sample dataset is provided at data/examples/EURUSD_5M_sample.csv for immediate testing.",
            fg="#777",
    ...
    def _browse_csv(self):
        ...
        if not result.is_valid:
            ...
            self.data_status.config(text="Import failed. See error dialog.", fg="red")
            return
        self.csv_path = path
        n = len(result.dataframe)
        warn = f" ({len(result.warnings)} warning(s))" if result.warnings else ""
        self.data_status.config(text=f"Loaded {os.path.basename(path)} — {n} bars{warn}", fg="green")

...
MAX_HOLD_BARS = 18
COOLDOWN_BARS = 6
# UPGRADE: the original strategy only traded a single 2.5-hour NY morning
# window (9:30-12:00 ET), which is why the real run only produced 8
# trades across the whole dataset -- nowhere near enough for a prop eval
# track record, whatever the edge. Added a second session (NY
# afternoon/London-NY overlap close, 13:30-16:00 ET) using the same
# entry logic, roughly doubling the opportunity count without touching
# what defines a qualifying setup. Both windows still respect
# MAX_HOLD_BARS / session-close flattening below.
...
def generate_signals(df: pd.DataFrame) -> pd.Series:
    ...
    in_session = (
        ((minute >= SESSION_1_START_ET) & (minute <= SESSION_1_END_ET))
        | ((minute >= SESSION_2_START_ET) & (minute <= SESSION_2_END_ET))
    )
    ...
    for i in range(n):
        ...
        # setup as the displacement/FVG. This uses its own (wider) window
        # from the FVG's own expiry, since the sweep often precedes the
        # confirming displacement/FVG by more than a couple of bars.
        recent_low_sweep = 0 <= i - last_sweep_low_bar <= SWEEP_RECENCY_BARS
        recent_high_sweep = 0 <= i - last_sweep_high_bar <= SWEEP_RECENCY_BARS
